...
请求 Body 使用 `dealer_info` 在 `db_sync_config` 中对应的 ID。本次配置中 `dealer_info` 对应 ID 为 `419`：
...
> [!warning] 请求 Body
> 如果这个节点只负责 `dealer_info`，应传 `dealer_info` 自己对应的 `db_sync_config` ID。不要直接把一次批量历史同步使用的多个 ID 原样复制进来。后续 ID 有变化时，以实际 `db_sync_config` 查询结果为准。
