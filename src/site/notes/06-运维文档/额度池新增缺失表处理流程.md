...
```mermaid
flowchart TD
    A[整理缺失表清单] --> B[FLINK 补 CDC 增量链路]
    B --> C[Dolphin 重启 cashPool-ri]
    C --> D[db_sync_config 补历史同步配置]
    D --> E[检查 sort_key / search_query / 并行度等特殊配置]
    E --> F[触发历史数据同步到 S3]
    F --> G[Code Builder 生成 Athena / DWD / ODS / Glue / Dolphin 产物]
    G --> H[生成内容放入 Redshift newframe]
    H --> I[提交 Redshift 代码]
    I --> J[启动或更新 Glue Crawler]
    J --> K[执行新增 Athena 表]
    K --> L[Redshift 创建 IODS]
    L --> M[Redshift 创建 DWD / View]
    M --> N[Dolphin 配置 s3->ods]
    N --> O[全链路验证]

    P[dealer_info 特殊处理] --> P1[离线数据获取工作流]
    P1 --> P2[remove s3 dealer_info]
    P2 --> P3[req_dealer_info]
    P3 --> P4[resp_dealer_info]
    P --> P5[s3->ods 中 dealer_info 使用专用子流程]
    P5 --> P6[dealer_info_all_history专用]

    Q[main_vice_company_relationship] -. 驼峰命名需单独检查 .-> M

    D -. dealer_info 需要额外配置 .-> P
    N -. dealer_info 进入专用链路 .-> P5
```

> [!note] 图中重点
> 主链路按本文 1～15 节展开；`dealer_info` 需要额外处理「离线数据获取」和 `dealer_info_all_history专用`；主副经销商相关表还要额外关注驼峰命名在 Redshift 洗数阶段的兼容问题。
