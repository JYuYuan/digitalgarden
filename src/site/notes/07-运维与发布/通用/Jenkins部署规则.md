---
{"dg-publish":true,"dg-permalink":"jenkins-deploy","permalink":"/jenkins-deploy/","dg-note-properties":{"permalink":"/jenkins-deploy/","cssclasses":["wiki-page","wiki-article"]}}
---


[[index\|首页]] / [[06-研发规范/开发规范\|研发规范]] / Jenkins 部署

# Jenkins 部署规则

基于 `shared-pipeline-library` 判断一次构建是否会部署。分支名含 `release` 或 `uat` 时只构建；其余分支由 `master` / `main` 规则或 `feature_branch_name` 精确匹配决定是否发版。

> [!wiki-nav] 本页导航
> [[07-运维与发布/通用/Jenkins部署规则#Jenkins 部署判断流程图\|判断流程]] · [[07-运维与发布/通用/Jenkins部署规则#核心参数说明\|配置参数]] · [[07-运维与发布/通用/Jenkins部署规则#常见配置示例\|环境示例]] · [[07-运维与发布/通用/Jenkins部署规则#分支部署行为\|行为对照]] · [[07-运维与发布/通用/Jenkins部署规则#实际使用流程\|操作步骤]]

## Jenkins 部署判断流程图

```mermaid
flowchart TD
    A["读取 Jenkinsfile 并执行 Node 构建"] --> B{"分支包含 release 或 uat？"}
    B -->|"是"| C["只构建，结束"]
    B -->|"否"| D{"分支是 master 或 main？"}
    D -->|"是"| E["推送镜像并部署 K8s"]
    D -->|"否"| F{"精确匹配 feature_branch_name？"}
    F -->|"是"| E
    F -->|"否"| G["不部署，结束"]
```
...
> [!important] 判断优先级
> `release` / `uat` 命名过滤先于强制发版判断；即使 `feature_branch_name` 精确匹配，也仍然只构建。Git 中的环境分支命名见 [[07-运维与发布/通用/Git分支规范#项目分支对照表\|项目分支对照表]]。
...
- [[07-运维与发布/通用/Git分支规范\|Git分支规范]] - 功能分支来源与环境差异
