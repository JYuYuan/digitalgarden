---
{"dg-publish":true,"dg-permalink":"git-branching","permalink":"/git-branching/","dg-note-properties":{"permalink":"/git-branching/","cssclasses":["wiki-page","wiki-article"]}}
---


[[index\|首页]] / [[06-研发规范/开发规范\|研发规范]] / Git 分支

# Git 分支规范

功能分支从生产分支创建，整理提交后分别合入目标环境。先确认项目的分支差异，再按开发、UAT、生产的顺序验证；实际部署条件见 [[07-运维与发布/通用/Jenkins部署规则\|Jenkins部署规则]]。

> [!wiki-nav] 本页导航
> [[07-运维与发布/通用/Git分支规范#分支创建规则\|创建分支]] · [[07-运维与发布/通用/Git分支规范#分支流转图\|流转图]] · [[07-运维与发布/通用/Git分支规范#项目分支对照表\|项目差异]] · [[07-运维与发布/通用/Git分支规范#不同场景的分支管理\|场景示例]] · [[07-运维与发布/通用/Git分支规范#注意事项\|操作检查]]
...
- 这三个分支是**长期分支**，不可删除
- 每个分支对应一个部署环境
- 功能开发从生产分支创建临时分支，见 [[07-运维与发布/通用/Git分支规范#分支创建规则\|#分支创建规则]]
...
**下表所列应用项目的功能分支从对应生产分支创建。**

额度池数据项目另有已记录的分支步骤：[[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#2.1 从 dev 分支切功能分支\|FLINK 从 dev 创建]]，[[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#10. Redshift 项目提交代码\|Redshift 按其提交步骤执行]]。
...
从生产分支创建可基于已稳定的代码，避免混入其他开发中的功能，并减少合并冲突。
...
图中展示操作顺序。每次合并的来源都是同一功能分支；测试通过后，再把该功能分支合入下一个目标环境。

### 标准项目

```mermaid
flowchart TD
    A["从 prod_release 创建功能分支"] --> B["功能开发并合并 commit"]
    B --> C["功能分支合入 dev"]
    C --> D["开发环境测试"]
    D -->|"通过"| E["同一功能分支合入 uat"]
    E --> F["UAT 测试"]
    F -->|"通过"| G["同一功能分支合回 prod_release"]
    G --> H["生产发布"]
```
...
```mermaid
flowchart TD
    A["从 sf_release 创建功能分支"] --> B["功能开发并合并 commit"]
    B --> C["功能分支合入 sf_uat"]
    C --> D["UAT 测试"]
    D -->|"通过"| E["同一功能分支合回 sf_release"]
    E --> F["生产发布"]
```
...
上面的箭头表示验证顺序。各环境都合入功能分支，完整 `git merge` 命令见 [[07-运维与发布/通用/Git分支规范#开发流程（包含 Commit 合并）\|#开发流程（包含 Commit 合并）]]。
...
**区分提交整理与共享分支合并：**
- 功能分支发版前的提交整理，按 [[07-运维与发布/通用/Git分支规范#开发流程（包含 Commit 合并）\|#开发流程（包含 Commit 合并）]] 使用交互式 rebase / squash。
- 多人共享分支的协作合并使用 `merge`，避免改写共享历史造成混乱。
...
- [[07-运维与发布/通用/Jenkins部署规则\|Jenkins部署规则]] - Jenkins 部署规则
- [[07-运维与发布/发布流程/SF直播App发布流程\|SF直播App发布流程]] - 移动端发布流程
