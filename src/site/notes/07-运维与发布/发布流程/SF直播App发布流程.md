...
用于 SF 直播 Android / RePlugin 在 UAT 和生产环境发版。每次先递增版本号，使用 JDK 8 生成 release JAR，再在对应后台上传并填写一致的数字版本号。
...
```mermaid
flowchart TD
    A["更新 versions.gradle 版本号"] --> B["登录 UAT 或生产后台"]
    B --> C["使用 JDK 8 打包 release JAR"]
    C --> D["版本管理 → 上传新版本"]
    D --> E["选择数字.jar，填写同一版本号"]
    E --> F["可选：更新说明与发布时间"]
    F --> G["检查文件与版本，确认提交"]
    G --> H["核对版本列表，可选下载测试"]
```

版本示例：`version_code = 13 → 14`、`version_name = "1.0.13" → "1.0.14"`；`plugin_version = version_code`，最终上传 `14.jar`、填写 `14`。完整打包命令与产物路径见 [[07-运维与发布/发布流程/SF直播App发布流程#步骤 2：准备发布包\|#步骤 2：准备发布包]]。
