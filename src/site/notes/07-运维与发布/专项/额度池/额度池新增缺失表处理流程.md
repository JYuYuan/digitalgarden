---
{"dg-publish":true,"dg-permalink":"cashpool-add-tables","permalink":"/cashpool-add-tables/","dg-note-properties":{"permalink":"/cashpool-add-tables/","cssclasses":["wiki-page","wiki-article"]}}
---


[[index\|首页]] / [[10-后端与数据项目/redshift\|数据项目]] / 额度池补表
...
> [!wiki-nav] 本页导航
> [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#1. 先整理本次缺失表清单\|表清单]] · [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#2. FLINK 项目补 CDC 增量链路\|CDC 增量]] · [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#4. 在 db_sync_config 中补历史数据配置\|历史同步]] · [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#8. 使用 Code Builder 生成相关文件\|生成产物]]
>
> [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#11. 启动 / 更新 Glue\|建表与调度]] · [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#14. Dolphin 配置 s3->ods 工作流\|Dolphin 与 dealer_info]] · [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#15. 最终验证\|验收]] · [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#操作检查表\|操作检查表]]

本页是操作步骤；本次生产需要同步的对象见 [[07-运维与发布/专项/额度池/额度池上线变更清单\|额度池上线变更清单]]，6 条 IODS 建表语句见 [[07-运维与发布/专项/额度池/额度池发版-IODS建表SQL\|额度池发版-IODS建表SQL]]。

## 整体处理流程

```mermaid
flowchart TD
    A["1 · 整理缺失表清单"] --> B["2–3 · 配置 CDC 并重启 FLINK"]
    B --> C["4–7 · 历史配置与同步到 S3"]
    C --> D["8 · Code Builder 生成文件"]
    D --> E["9–10 · newframe 入库并合并代码"]
    E --> F["11–13 · Glue、Athena 与 Redshift 建表"]
    F --> G["14 · 配置 Dolphin 工作流"]
    G --> H["15 · 分别验证增量与历史链路"]
```

图示为实施顺序。CDC 增量与历史同步各自有配置和验证要求，两条链路都正常才算完成，见 [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#15. 最终验证\|#15. 最终验证]]。
...
> [!tip] 确认目标工作流
> 按上方路径找到 `cashPool-ri` 后再执行下线 / 上线，避免操作错工作流。
...
- 数据库与数据库映射正确
- `source.table.list` 与本次缺失表清单一致，共 **17 张**
- 没有漏表，也没有混入无关表
- 后续需要的 `iods / dwd / dolphin / glue / Athena` 生成内容正常

本次额度池已成功预览并处理 **17 张表**。这里记录预览校验数量；[[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#14.3 本次新增表顺序\|#14.3 本次新增表顺序]] 中的 **6 张表** 是需要补入 `s3->ods` 的范围，核对时分别使用对应清单。
...
导出 ZIP 后，继续 [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#9. 把 Code Builder 生成内容放到 Redshift 的 newframe 目录\|放入 newframe]]；生成前的输入配置与预览检查按本节 8.1–8.3 执行。
...
> [!important] 生成文件落点
> 在 `cashpool` 下创建独立的 `newframe` 目录，把 Code Builder ZIP 解压目录里的**全部内容**复制进去，保持目录层级和文件结构不变。不要覆盖 `cashpool` 原有的 `iods / dwd / glue / dolphin` 等目录。
...
[[07-运维与发布/专项/额度池/额度池发版-IODS建表SQL\|额度池发版-IODS建表SQL]]
...
实施顺序见 [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#整体处理流程\|#整体处理流程]]，逐项验收使用下方 [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#操作检查表\|#操作检查表]]。生产变更范围及特殊工作流由 [[07-运维与发布/专项/额度池/额度池上线变更清单\|额度池上线变更清单]] 汇总。
...
| 阶段 | 操作 | 完成标准 |
|---|---|---|
| 1 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#1. 先整理本次缺失表清单\|整理缺失表]] | 有完整表清单 |
| 2 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#2. FLINK 项目补 CDC 增量链路\|FLINK CDC]] | dev 切分支并补表 |
| 3 | Git | 合并到 dev |
| 4 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#3. 重启 Dolphin 中的 FLINK 工作流\|Dolphin FLINK]] | cashPool-ri 下线 → 上线 |
| 5 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#5. 推荐的 SQL 新增方式\|db_sync_config]] | SQL 克隆模板配置 |
| 6 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#6. db_sync_config 特别检查项\|配置检查]] | sort_key / search_query 正确 |
| 7 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#5.2 多表新增\|获取 ID]] | 已拿到新增 ID |
| 8 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#7. 触发历史数据同步到 S3\|历史同步]] | POST sync/db |
| 9 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#执行后检查\|S3 检查]] | 历史数据已落 S3 |
| 10 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#8. 使用 Code Builder 生成相关文件\|Code Builder]] | ZIP 已生成 |
| 11 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#9. 把 Code Builder 生成内容放到 Redshift 的 newframe 目录\|Redshift 项目]] | cashpool 下创建 newframe，并放入 Code Builder 解压目录的全部内容 |
| 12 | Git | MR → master |
| 13 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#11. 启动 / 更新 Glue\|Glue]] | Job 正常 |
| 14 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#12. 执行新增 Athena 表\|Athena]] | 新表可查询 |
| 15 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#13. Redshift 创建 IODS / DWD / View\|Redshift]] | IODS / DWD / View 已创建 |
| 16 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#14. Dolphin 配置 s3->ods 工作流\|Dolphin]] | s3->ods 已配置 |
| 17 | [[07-运维与发布/专项/额度池/额度池新增缺失表处理流程#15. 最终验证\|最终验证]] | 历史 + 增量都正常 |
